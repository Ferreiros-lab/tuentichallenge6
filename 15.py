...
def get_probability(matrix, exponent, initial):
    ...
    f = Fraction(max_value, 100**exponent)
...
def main():
    ...
    markov = numpy.matrix(matrix, dtype = 'object')
    Q = int(raw_input())
    for num_case in range(1, Q + 1):
        ...
        if changes < 200000:  # Small enough to compute the whole matrix
            numerator, denominator, index = get_probability(markov,
                                                            changes,
                                                            initial)
        else:  # The exponent is huge, but the patterns are "almost" cyclical
            cycleLength = 4  # Try with many cycle lenght, and get the most common
            numerators = Counter()
            denominators = Counter()
            for factor in range(2, 10):
                cycleLength = lcm(cycleLength, factor)
                # Adding cycleLength to avoid the noise of the first elements
            ...
            numerator = numerators.most_common()[0][0]
            denominator = denominators.most_common()[0][0]
            # Since the maximum index converges quickly we only keep the last one
            
        print 'Case #%d: Chair: %d Last digits: %d/%d' % (
            num_case, index, numerator, denominator)
